...
- VBA-Agreement: No two nonfaulty, directly connected nodes accept different values.
- VBA-Rejection: If any nonfaulty node $$\mathcal{P}_i$$ accepts $$1$$, then for every node $$\mathcal{P}_j$$ directly connected to $$\mathcal{P}_i$$, there is some thread $$S\in N_j$$ such that at least $$f_S+1$$ nonfaulty nodes entered $$\mathsf{VALIDATING}$$ mode.
- VBA-Termination: Every nonfaulty, unsplit node eventually terminates with probability $$1$$.
