...
Given a thread $$S$$, let $$f_S=\lfloor(\vert S\vert-1)/3\rfloor$$. A node is called **nonfaulty** if it behaves according to the protocol, and does not stay halted forever. A thread $$S$$ is called **nonfaulty** if the number of *faulty* nodes in $$S$$ is no greater than $$f_S$$. A thread $$S$$ is **healthy** if the number of unhealthy nodes in $$S$$ is no greater than $$f_S$$, where a node is **healthy** if it is nonfaulty and all of its threads are healthy. Although this definition appears circular at first, it can be made noncircular by defining a sequence of sets $$F_n$$ for each natural number $$n\in\mathbb{N}$$, where $$F_1$$ is the set of faulty nodes, and $$F_n$$ is the union of $$F_{n-1}$$ with the set of all nodes $$\mathcal{P}_i$$ such that $$\vert S\cap F_{n-1}\vert\geqslant f_S+1$$ for some thread $$S\in N_i$$. Then the set of unhealthy nodes is simply defined to be the union of the $$F_n$$ across all $$n\in\mathbb{N}$$.

We say that $$\mathcal{P}_i$$ and $$\mathcal{P}_j$$ are **directly connected** if there is at least one nonfaulty thread in $$N_i\cap N_j$$. We say that $$\mathcal{P}_i$$ is **strongly connected to** $$\mathcal{P}_j$$ if for every nonfaulty node $$\mathcal{P}_k\in N_i$$, $$\mathcal{P}_j$$ and $$\mathcal{P}_k$$ are directly connected. We say that $$\mathcal{P}_i$$ is **split** if there are two nodes $$\mathcal{P}_j,\mathcal{P}_k\in N_i$$ such that $$\mathcal{P}_j$$ and $$\mathcal{P}_k$$ are *not* directly connected, and $$\mathcal{P}_i$$ is **unsplit** otherwise. We say that $$\mathcal{P}_i$$ is **fully unsplit** if every pair of nodes $$\mathcal{P}_j,\mathcal{P}_k$$ in $$\mathcal{P}_i$$'s *extended* safety net is directly connected. It is easily seen that any node in the extended safety net of a fully unsplit node is itself fully unsplit. Also note that a fully unsplit node is automatically strongly connected to every node in its extended safety net.
...
- RBC-Agreement: No two nonfaulty, directly connected nodes accept different messages.
- RBC-Termination-Agreement: If a nonfaulty node $$\mathcal{P}_i$$ accepts some message, then every other healthy, fully unsplit node directly connected with $$\mathcal{P}_i$$ will eventually ready that same message.
- RBC-Strong-Termination-Agreement: If a nonfaulty node $$\mathcal{P}_i$$ accepts some message, then every other healthy, fully unsplit node *strongly* connected to $$\mathcal{P}_i$$ will eventually *accept* that same message.

Binary agreement has the following properties:
- BA-Validity: If a healthy node $$\mathcal{P}_i$$ accepts some value, a healthy node in $$N_i^{\infty}$$ must have proposed that value.
- BA-Agreement: No two nonfaulty, directly connected nodes accept different values.
- BA-Termination: Every nonfaulty, unsplit node eventually terminates with probability $$1$$.
...
At the beginning of the protocol, the node $$\mathcal{P}_i$$ creates an instance of $$\mathsf{VBA}_b$$ and $$\mathsf{RBC}_b$$ for every node $$\mathcal{P}_b\in\overline{B}_i$$. If a node $$\mathcal{P}_b$$ is not in $$\overline{B}_i$$, then $$\mathcal{P}_i$$ can refuse to broadcast messages (i.e., refuses to participate in reliable broadcast or binary agreement) pertaining to $$\mathcal{P}_b$$ without causing any issues. In practice, if we hear messages pertaining to a node which is not in $$\overline{B}_i$$, then that's probably a sign that either 1) the person broadcasting those messages is unhealthy, or 2) we're not fully unsplit, and we should fix our safety net to deal with that.
...
1. Upon accepting a hash value from $$\mathsf{RBC}_b$$, check if we know the transaction set that it references. If not, ask our peers if they know to try to find it out. After learning the transaction set which gave rise to the hash, if $$\mathcal{P}_b$$ is in $$B_i$$ then vote $$1$$ in $$\mathsf{BA}_b$$.
2. Once at least $$R$$ instances of $$\mathsf{BA}$$ have terminated (possibly not on $$1$$), thereafter automatically vote $$0$$ for every other instance of $$\mathsf{BA}$$. Additionally, for any instance of $$\mathsf{BA}$$
3. Once all instances of $$\mathsf{BA}$$ relating to all nodes in $$\overline{B}_i$$ have terminated, let $$T$$ be the set of nodes in $$\overline{B}_i$$ which terminated on $$1$$.
4. Wait until we have readied some message under reliable broadcast from every node in $$T$$. If we ready a hash that we haven't heard from, ask our peers to try to find it out; by BA-Validity, there must have been some healthy node in extended safety net which voted yes on that instance, and, due to the criterion for voting yes in step $$1$$, this means they must have the transaction set. Thus we can get the transaction set from them; since they're in our extended safety net, it suffices to simply keep asking for it from the nodes in our immediate safety net, since some healthy node in our immediate safety will eventually receive it the same way. Because we have the hash, a Byzantine node cannot trick us into taking the wrong transaction set, and we don't need any confirmations to know we have the correct set.
...
- Safety: If a nonfaulty node adds $$\mathcal{P}_b$$ to its ballot, then every other nonfaulty, directly connected node will add $$\mathcal{P}_b$$ to its tablist before terminating the protocol.
- Termination: If a node is healthy and fully unsplit, then eventually it will terminate.

Proof of safety: Suppose a nonfaulty node $$\mathcal{P}_i$$ adds a node $$\mathcal{P}_b$$ to $$B_i$$. Then for every node $$\mathcal{P}_j$$ directly connected with $$\mathcal{P}_i$$, there must be some thread $$S\in N_j$$ such that at least $$f_S+1$$ nonfaulty nodes in $$S$$ broadcasted support for $$\mathcal{P}_b$$ in their ballot messages. Thus $$\mathcal{P}_j$$ won't move past step $$3$$ until adding $$\mathcal{P}_b$$ to $$\overline{B}_j$$. □

Proof of termination: Suppose $$\mathcal{P}_i$$ is healthy and fully unsplit. Then if $$\mathcal{P}_j\in N_i^{\infty}$$ is a nonfaulty node which broadcasts support for a node $$\mathcal{P}_b$$, then $$\mathcal{P}_j$$ must have accepted a reliable broadcast from $$\mathcal{P}_b$$, so by RBC-Strong-Termination-Agreement, eventually $$\mathcal{P}_i$$ will also accept a reliable broadcast from $$\mathcal{P}_b$$ and add $$\mathcal{P}_b$$ to $$\overline{B}_i$$. Thus eventually $$\overline{B}_i$$ will contain the ballot suggestion of every nonfaulty node in its extended safety net; hence in particular it will contain the ballot suggestion of every nonfaulty node in its safety net. Since $$\mathcal{P}_i$$ is healthy by assumption, at least $$2f_S+1$$ nodes in every thread $$S\in N_i$$ are healthy, so eventually $$\mathcal{P}_i$$ will satisfy the predicate in step $$3$$ and terminate. □

## Implementing the Primitives

Here we describe finally how the primitives themselves work. There are actually three primitives that are needed: binary proposal ($$\mathsf{BPP}$$), validated binary agreement ($$\mathsf{BA}$$), and reliable broadcast ($$\mathsf{RBC}$$). $$\mathsf{BPP}$$ is used to construct $$\mathsf{BA}$$, and $$\mathsf{BA}$$ and $$\mathsf{RBC}$$ are used in conjunction to construct the overlying multi-valued consensus protocol.
...
- BPP-Agreement: If any nonfaulty node $$\mathcal{P}_i$$ adds a bit $$v$$ to $$\mathsf{values}_i$$, then every other healthy node $$\mathcal{P}_j$$ strongly connected to $$\mathcal{P}_i$$ will eventually add $$v$$ to $$\mathsf{values}_j$$.
- BPP-Progression: If $$\mathcal{P}_i$$ is healthy and unsplit, then eventually $$\mathsf{values}_i$$ is nonempty.
...
RBC-Validity follows by the same general argument as BPP-Validity. RBC-Agreement follows from the fact that no nonfaulty thread can strongly support two contradictory statements.

Proof of RBC-Termination-Agreement:
Suppose $$\mathcal{P}_j$$ is healthy and fully unsplit. Suppose a healthy node in $$N_j^{\infty}$$ broadcasts $$\mathsf{READY}(v)$$ for some message $$v$$. Let $$\mathcal{P}_k$$ be the first healthy node in $$N_j^{\infty}$$ to broadcast a ready message for $$v$$. Since $$N_k\subseteq N_j^{\infty}$$ by definition, $$\mathcal{P}_k$$ must have broadcasted ready before any healthy node in $$N_k$$, so in particular $$\mathcal{P}_k$$ could not have broadcasted ready due to seeing weak support for $$\mathsf{READY}(v)$$, because $$\mathcal{P}_k$$ is healthy by assumption. Thus $$\mathcal{P}_k$$ must have broadcasted ready due to seeing strong support for $$\mathsf{ECHO}(v)$$. But any two healthy nodes $$N_j^{\infty}$$ are by definition directly connected, and thus cannot both see strong support for contradictory messages. Thus no two healthy nodes in $$N_j^{\infty}$$ can ready contradictory messages.

Now if a nonfaulty node $$\mathcal{P}_i$$ directly connected with $$\mathcal{P}_j$$ accepts a message $$v$$, then $$\mathcal{P}_j$$ will eventually see weak support for $$\mathsf{READY}(v)$$. Since $$\mathcal{P}_j$$ is healthy by assumption, there must have been some healthy node in one of its threads which broadcasted $$\mathsf{READY}(v)$$; by the previous paragraph, $$\mathcal{P}_j$$ cannot have already broadcasted $$\mathsf{READY}(w)$$ for any message $$w\neq v$$. Thus eventually $$\mathcal{P}_j$$ will broadcast $$\mathsf{READY}(v)$$ by the predicate in step $$2$$. □

RBC-Strong-Termination-Agreement follows from RBC-Termination-Agreement by definition of strong connectivity and the fact that any node in the safety net of a fully unsplit node is itself fully unsplit: since thus eventually every healthy node in $$N_j$$ will ready the message, causing $$\mathcal{P}_j$$ to see strong support for that message because it's healthy by hypothesis.

#### Binary Agreement (based on Mostefaouil's protocol)
...
I'll prove the BA properties in another note; they require more effort to prove than the BPP and RBC properties, and trying to cram the proofs into this already-huge note would just be overkill. Also I'm not sure BA is in its final form; my intuition tells me I can make it work better in incomplete graphs with a bit more work.


## Regarding the Common Coin

The binary agreement protocol by Mostefaouil (which BA is mostly a copy of) uses a "common coin", which is basically a threshold signature scheme used as a source of random bits that all honest nodes agree upon, but for which the Byzantine nodes are unable to predict until at least one honest node has requested the coin's value. The neccessity of such a common coin comes from the assumption that the "adversary" (i.e., the entity that is assumed to be controlling all of the Byzantine nodes in order to wreak optimum havok on the network) controls when messages are delivered between honest nodes. Without the coin, the adversary can make messages be delivered in an unoptimal order to prevent the algorithm from ever terminating. The common coin makes it so that the adversary has no way to know when to deliver messages in order to prevent termination.
...
Regardless, we still need a way to get coin values for each round which are agreed upon by all honest nodes. An alternative is to simply use the round number mod $$2$$. This has the benefit of being very simple, and (since round numbers start at $$1$$) if all honest nodes propose $$1$$ then consensus will always terminate on the first round. Since the multi-valued algorithm needs to wait until a certain number of BA have terminated on $$1$$ before nodes can start voting $$0$$, this speeds up the time it takes for nodes to move onto the next step. It also biases the council size towards being larger, which seems like a good thing.
