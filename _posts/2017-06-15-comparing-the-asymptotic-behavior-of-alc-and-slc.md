...
The familiar definition of convergence of an infinite sequence says that $$T^n$$ **converges** to $$T^{\star}$$ iff for all $\varepsilon>0$ there exists some $$N\in\mathbb{N}$$ such that for all $$n>N$$, $$|T^n-T^{\star}|$$ (here we're using the matrix norm defined in the notation section). There is another unfortunately convoluted definition that we will find useful near the end of this note: we say that $$T^n$$ **binomially converges** to $$T^{\star}$$ iff for all $\varepsilon>0$ there exists some $$N\in\mathbb{N}$$ such that for any $$0<p<1$$, $$\lim\limits_{n\to\infty}\sum_{i=N+1}^n\binom{n}{i}(1-p)^{n-i}p^i|T^i-T^{\star}|<\varepsilon$$. If $$T^n$$ converges to $$T^{\star}$$, then using the $$N$$ given by definition of convergence, we have
$$$\begin{aligned}
\lim\limits_{n\to\infty}\sum_{i=N+1}^n\binom{n}{i}(1-p)^{n-i}p^i|T^i-T^{\star}|&<\lim\limits_{n\to\infty}\sum_{i=N+1}^n\binom{n}{i}(1-p)^{n-i}p^i\varepsilon\\
...
$$$\begin{aligned}
x_n^v&=E[X_n^v]\\
&=E\left[(1-P(v))X_{n-1}^v+P(v)\left(\frac{1}{|N^v|}\sum_{u\in N^v} X_{n-1}^u\right)\right]\\
&=(1-P(v))E[X_{n-1}^v]+P(v)\left(\frac{1}{|N^v|}\sum_{u\in N^v} E[X_{n-1}^u]\right)\\
&=(1-P(v))x_{n-1}^v+P(v)\left(\frac{1}{|N^v|}\sum_{u\in N^v} x_{n-1}^u\right)\phantom{----} (eq.1)
\end{aligned}$$$
...
$$$\begin{aligned}
c_n=\max\{\left|\frac{P^i}{i!(1-P)^i}T^i\right|:1\leqslant i\leqslant k\}.
\end{aligned}$$$
...
Thus,
$$$\begin{aligned}
\left|\sum_{i=1}^{k}\binom{n}{i}(1-P)^{n-i}P^iT^i\right|&\leqslant\sum_{i=1}^{k}\left|\binom{n}{i}(1-P)^{n-i}P^iT^i\right|\\
...
$$$\begin{aligned}
\lim\limits_{n\to\infty}\left|\sum_{i=1}^{k}\binom{n}{i}(1-P)^{n-i}P^iT^i\right|=0.
\end{aligned}$$$

But by definition of the norm $$|\cdot|$$, this says exactly that the partial sum converges to the zero matrix. Thus, for any $$k\in\mathbb{N}$$ we can cut out the first $$k$$ terms of the sum in $$(eq.1)$$ without changing the asymptotic behavior of the sum, i.e.,
$$$\begin{aligned}
...
Given any $$\varepsilon>0$$, we can now show that $$|T^{\star}-\tilde{T}^{\star}|<\varepsilon$$, from which it immediately follows that $$|T^{\star}-\tilde{T}^{\star}|=0$$ which implies $$T^{\star}=\tilde{T}^{\star}$$. Since $$T^{\star}=\lim\limits_{n\to\infty}T^n$$, by definition of limits, for any $$\varepsilon>0$$ there exists $$N\in\mathbb{N}$$ such that for any $$n>N$$, $$|T^{\star}-T^n|<\varepsilon$$.

Here is where we need binomial convergence of $$T^n$$. Let $$N$$ be large enough such that $$\lim\limits_{n\to\infty}\sum_{i=N+1}^{n}\binom{n}{i}(1-P)^{n-i}P^i\left|T^i-T^{\star}\right|<\varepsilon$$. Then
$$$\begin{aligned}
|\tilde{T}^{\star}-T^{\star}|&=\lim\limits_{n\to\infty}\left|\left(\sum_{i=N+1}^{n}\binom{n}{i}(1-P)^{n-i}P^iT^i\right)-T^{\star}\right|\\
&\mathop{=}\limits^{(eq.2)}\lim\limits_{n\to\infty}\left|\sum_{i=N+1}^{n}\binom{n}{i}(1-P)^{n-i}P^i\left(T^i-T^{\star}\right)\right|\\
&\leqslant\lim\limits_{n\to\infty}\sum_{i=N+1}^{n}\binom{n}{i}(1-P)^{n-i}P^i\left|T^i-T^{\star}\right|\\
&<\varepsilon.
\end{aligned}$$$
